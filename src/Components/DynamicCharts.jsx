...
const DynamicCharts = () => {
  ...
  useEffect(() => {
    async function fetchChartDetails() {
      try {
        setIsLoading(true);
        const cachedData = sessionStorage.getItem("@dynamicChartData");
        const data = cachedData
          ? JSON.parse(cachedData)
          : await genrateChartData();

        if (!cachedData)
          sessionStorage.setItem("@dynamicChartData", JSON.stringify(data));
          ...
          {chartData.map((chartOptions, idx) => (
            <ErrorBoundary key={idx} fallback={<SomethingWentWrong />}>
              <Suspense fallback={<Loading size={50} />}>
                ...
                >
                  <Chart options={chartOptions} isDynamic={true} />
                </div>
              </Suspense>
            </ErrorBoundary>
          ))}
...
export default memo(DynamicCharts);
